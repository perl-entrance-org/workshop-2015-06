...
- 以下の機能がある`YAPC`モジュールを実装してください
    ...
    + `YAPC::year()`で年(2015)を4桁の整数で返します（テストをAの人が、コードをBの人が書きましょう）
    + `YAPC::month()`で月(8)を2桁の整数で返します（テストをBの人が、コードをAの人が書きましょう）
    + `YAPC::day()`で開催日初日の日付(20)を2桁の整数で返します（テストをAの人が、コードをBの人が書きましょう）
